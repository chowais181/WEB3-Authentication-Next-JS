...
const Home: NextPage = () => {
  ...
  const authenticatedRequest = async () => {
    ...
    if (res.ok) {
      ...
      setAuthMessage(
        `Successfully authenticated to backend with address ${address}`
      );
    } else {
      setAuthMessage(
  ...
  return (
    <div className="main">
      ...
      {address ? (
        <>
          {isLoggedIn ? (
            <button onClick={logoutWallet} className="btn">
              Logout
            </button>
          ) : (
            <button onClick={signInWithEthereum} className="btn">
              Login with Wallet
            </button>
          )}

          <button onClick={authenticatedRequest} className="btn">
            Authenticate
          </button>
